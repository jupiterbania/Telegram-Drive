...
import { X, RotateCcw, Download, Upload, Trash2, HardDrive, Globe, Key, Copy, Check, RefreshCw, FolderArchive, Shield, Zap, Activity, Gauge, Wifi, ChevronDown, Link, Sparkles, Info, Monitor, Loader2, Languages, Play, Palette, Tag, Search, Bug, Database, FolderSync, ArrowUpDown } from 'lucide-react';
import { SORT_OPTIONS } from '../../../utils/fileSorting';
                ...
                >
                    ...
                    >
                        ...
                        <div className="relative min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-7 py-6">
                            ...
                            {/* File Sorting & Display Section */}
                            <section className="space-y-3">
                                <h3 className="text-xs font-semibold text-telegram-subtext uppercase tracking-wider flex items-center gap-2">
                                    <ArrowUpDown className="w-3.5 h-3.5" />
                                    {t('settings.file_sorting', 'File Sorting')}
                                </h3>

                                <SettingsRow
                                    icon={<ArrowUpDown className="h-4 w-4 text-telegram-subtext" />}
                                    title={t('settings.default_sort', 'Default File Sort')}
                                    description={t('settings.default_sort_desc', 'Choose which files appear at the top by default')}
                                    control={(
                                        <select
                                            ...
                                            onChange={(event) => {
                                                ...
                                                if (selected) {
                                                    updateSettings({
                                                        fileSortField: selected.field,
                                                        fileSortDirection: selected.direction,
                                                    });
                                                }
                                            }}
                        ...
                        {activeTab === 'vpn' && (
                                ...
                                {activeTab === 'sharing' && (
                                    <SharingSettingsTab>
                                        ...
                                        <div className="bg-telegram-hover/30 border border-telegram-border/50 rounded-lg p-3 space-y-2">
                                            <div className="text-[11px] font-semibold text-telegram-text flex items-center justify-between">
                                                <span className="flex items-center gap-1">☁️ 24x7 Cloud Share Worker URL</span>
                                                <span className="text-[10px] text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded font-medium">Free Cloudflare</span>
                                            </div>
                                            <input
                                                type="url"
                                                placeholder="e.g. https://telegram-drive-cloud-share.yourname.workers.dev"
                                                value={cloudWorkerUrl}
                                                onChange={(e) => {
                                                    const val = e.target.value;
                                                    setCloudWorkerUrl(val);
                                                    localStorage.setItem('telegram_drive_cloud_worker_url', val.trim());
                                                }}
                                                className="w-full bg-telegram-surface border border-telegram-border rounded-md px-2.5 py-1.5 text-xs text-telegram-text focus:outline-none focus:border-telegram-primary/50 placeholder:text-telegram-subtext/40"
                                            />
                                            <p className="text-[10px] text-telegram-subtext">
                                                Enter your Cloudflare Worker URL to enable 24x7 password-protected links that work even when you are offline or this app is closed.
                                            </p>
                                        </div>
