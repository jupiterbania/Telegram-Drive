...
import { X, ArrowLeft, AlertTriangle, Loader2, RefreshCw, StopCircle, Maximize2, Minimize2, Volume2, VolumeX, Volume1, Play, Activity, Trash2, Zap } from 'lucide-react';
...
import { getCachedThumbnail, setCachedThumbnail } from '../../../services/imagePreviewCache';
import { extractFrameFromVideoElement } from '../../../services/videoThumbnailService';
...
export function AdaptiveMediaPlayer({
    ...
    const markPlaybackConfirmed = useCallback((e?: React.SyntheticEvent<HTMLVideoElement>) => {
        playbackConfirmedRef.current = true;
        setPlaybackConfirmed(true);
        setOriginalPlaybackError(null);

        // Auto-capture thumbnail on playback if missing
        const activeFolder = activeFolderId ?? null;
        if (!getCachedThumbnail(file.id, activeFolder)) {
            const video = (e?.currentTarget as HTMLVideoElement | null) || hlsVideoRef.current || mseVideoRef.current;
            if (video && video.videoWidth > 0) {
                const dataUrl = extractFrameFromVideoElement(video);
                if (dataUrl) {
                    setCachedThumbnail(file.id, activeFolder, dataUrl);
                }
            }
        }
    ...
    const handleUserActivity = useCallback(() => {
        ...
        controlsTimeoutRef.current = window.setTimeout(() => {
            const video = hlsVideoRef.current || mseVideoRef.current || document.querySelector('video');
    ...
    return (
        ...
        >
            <div ref={containerRef} className="relative w-full h-full flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
                ...
                >
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                            <h2 className="truncate text-xs sm:text-sm font-semibold text-white max-w-[170px] sm:max-w-md md:max-w-xl" title={file.name}>
                                {file.name}
                            </h2>
                            <p className="text-[10px] sm:text-xs text-white/50 flex items-center gap-2">
                                ...
                                {(sourceResolution || playingResolution) && (
                                    <span>• {(playingResolution || sourceResolution)?.w}×{(playingResolution || sourceResolution)?.h}</span>
                                )}
                            </p>
                        </div>
                    </div>
                ...
                {/* Floating Bottom Overlay Toolbar */}
                {(displayPhase !== 'error' && displayPhase !== 'failed') && (
                    <div
                        className={`pointer-events-none absolute inset-x-0 bottom-0 z-30 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4 pt-12 pb-[max(1rem,env(safe-area-inset-bottom,1rem))] transition-all duration-200 ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}
                    >
                        <div className="flex items-center justify-between pointer-events-auto">
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/10 hover:bg-white/20 active:scale-95 text-white flex items-center justify-center transition-all border border-white/10"
                                ...
                                >
                                    <button onClick={toggleMute} className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white/80 flex items-center justify-center transition-all border border-white/10" title={isMuted ? 'Unmute' : 'Mute'}>
                                ...
                                {/* File name */}
                                <span className="text-xs sm:text-sm text-white/80 truncate max-w-[160px] sm:max-w-[300px]">{file.name}</span>
                            </div>

                            <div className="flex items-center gap-2">
                                <button
                                    onClick={toggleFullscreen}
                                    className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white/80 flex items-center justify-center transition-all border border-white/10"
